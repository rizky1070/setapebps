			...
			<div class="card full-height">
				<div class="card-header">
					<div class="card-head-row">
						...
						<a href="{{ route('sekretariat.index') }}" class="btn btn-primary btn-sm ml-auto">Back</a>
					</div>
				</div>
				
				<div class="card-body">
					<form action="{{ route('sekretariat.update', $ketua->id) }}" method="POST" enctype="multipart/form-data">
						@method('PUT')
						@csrf
                        ...
                        <select class="form-control" name="category_id" id="">
                            ...
                            @foreach ($sortedCategories as $cat)
                                <option value="{{ $cat->id }}" {{ $ketua->category_id == $cat->id ? 'selected' : '' }}>
                                    {{ $cat->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select class="form-control" id="type" name="status">
                            ...
                            @foreach ($statuses as $status)
                                <option value="{{ $status['value'] }}" {{ $ketua->status == $status['value'] ? 'selected' : '' }}>
                                    {{ $status['label'] }}
                                </option>
                            @endforeach
                        </select>
