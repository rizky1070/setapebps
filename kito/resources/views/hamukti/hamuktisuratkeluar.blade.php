            ...
            <div class="flex flex-col flex-1 overflow-hidden">
                ...
                <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
                    <div class="container px-6 py-8 mx-auto">
                        <h3 class="text-3xl font-medium text-gray-700">Surat Keluar</h3>
                        <div class="mt-8">
                            <div class=" mb-3">
                                <div class="w-full md:w-1/2 flex flex-col md:flex-row">
                                    <div class="flex flex-grow mb-2 md:mb-0">
                                        <input type="text" id="searchInput" class="w-full border border-gray-300 rounded p-2" placeholder="Cari berdasarkan perihal atau nama instansi">
                                ...
                                <div class="w-full md:w-1/2 flex items-center space-x-2 mt-3">
                                    ...
                                    <!-- Tombol Reset -->
                                    <a id="resetButton" href="/hamuktisuratkeluar" class="ml-2 px-4 py-2 bg-gray-300 text-black rounded-lg hover:bg-gray-400">
                                        Reset
                                    </a>
                                @if (auth()->user()->is_leader)
                                <a href="{{ URL('hamuktisuratkeluarform') }}" class=" bg-green-500 text-white rounded-md px-4 py-2">{{__('Tambah')}}</a>
                                @else
                                    {{-- <a href="{{ URL('hamuktisuratkeluarform') }}" class=" bg-green-500 text-white rounded-md px-4 py-2">{{__('Tambah')}}</a> --}}
                                @endif
                                </div>
                                ...
                                <div
                                    ...
                                    <table class="min-w-full">
                                        ...
                                        <tbody class="bg-white">
                                            ...
                                            <tr id="{{ $suratkeluar['id'] }}">
                                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                    <div class="text-sm font-medium leading-5 text-gray-900 whitespace-nowrap">{{ $suratkeluar['nomorfull'] }}
                                                    </div>
                                                ...
                                                <td class="relative mx-auto max-w-6xl [--arrow-size:5px] [--tooltip-color:white] border-b border-gray-200">
                                                    <div id="grid" class=" gap-2  p-4 ">
                                                        <div id="gridItem" class="relative cursor-pointer min-w-40 line-clamp-2 text-xs hover:z-50" data-tooltip="LOOK HERE">{{ $suratkeluar['perihal'] }}</div>
                                                    </div>
                                                ...
                                                <td class="relative mx-auto max-w-6xl [--arrow-size:5px] [--tooltip-color:white] border-b border-gray-200">
                                                    <div id="grid" class=" gap-2  p-4 ">
                                                        <div id="gridItem" class="relative cursor-pointer min-w-40 line-clamp-2 text-xs hover:z-50" data-tooltip="LOOK HERE">{{ $suratkeluar['tanggal'] }}</div>
                                                    </div>
    
                                                </td>
                                                <td class="relative mx-auto max-w-6xl [--arrow-size:5px] [--tooltip-color:white] border-b border-gray-200">
                                                    <div id="grid" class=" gap-2  p-4 ">
                                                        <div id="gridItem" class="relative cursor-pointer min-w-40 line-clamp-2 text-xs hover:z-50" data-tooltip="LOOK HERE">{{ $suratkeluar['namainstansi'] }}</div>
                                                    </div>
                                                ...
                                                <td
                                                    ...
                                                    @if (auth()->user()->is_leader)
                                                    <form action="{{ route('hamuktisuratkeluar.delete', $suratkeluar['id']) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus ini?');" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
                ...
                <div class="p-4 md:p-5 space-y-4 max-h-96 overflow-y-auto">
                    ...
                    <div>
                        <a id="download" href="" download>
                            <i class="fas fa-download"></i> Download File Surat
                        </a>
                    </div>
                    ...
                    <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                        @if (auth()->user()->is_leader)
                        <button id="edit-button" onclick="window.location.href='/hamuktisuratkeluarformedit/' + document.getElementById('id').textContent;" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                        Edit
                    </button>
                    <button id="hapus-button" type="button" class="ms-3 text-white bg-red-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                        Hapus
                    </button>
                    @else
...
<script type="text/javascript">
    ...
    detailButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            fetch(`/hamuktisuratkeluar/${rowId}`) // Ganti URL sesuai dengan endpoint Anda
                .then(response => {
                    if (!response.ok) {
                ...
                .then(eventData => {
                    ...
                    modalTitle.textContent = eventData.perihal; // Mengganti judul modal dengan judul event

                    var modalId = modal.querySelector('#id');
                    modalId.textContent = eventData.id;

                    var modalDateStart = modal.querySelector('#tanggal'); // Pastikan ada elemen dengan ID ini di modal
                    modalDateStart.textContent = eventData.tanggal; // Mengganti teks tanggal mulai

                    var modalDateEnd = modal.querySelector('#nomorfull'); // Pastikan ada elemen dengan ID ini di modal
                    modalDateEnd.textContent = eventData.nomorfull; // Mengganti teks tanggal selesai

                    var modalTimeStart = modal.querySelector('#perihal'); // Pastikan ada elemen dengan ID ini di modal
                    modalTimeStart.textContent = eventData.perihal; // Mengganti teks tanggal mulai

                    var modalFile = modal.querySelector('#download');
                    var filePath = eventData.file;
                    modalFile.setAttribute('href', '{{ asset('/storage/uploads/docs') }}/' + filePath);
