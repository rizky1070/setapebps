            ...
            <div class="flex flex-col flex-1 overflow-hidden">
                ...
                <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
                    <div class="container px-6 py-8 mx-auto">
                        <h3 class="text-3xl font-medium text-gray-700">SK</h3>
                        <div class="mt-8">
                            <div class=" mb-3">
                                ...
                                <div class="w-full md:w-1/2 flex items-center space-x-2 mt-3">
                                    ...
                                    <!-- Tombol Reset -->
                                    <a id="resetButton" href="/hamuktisk" class="ml-2 px-4 py-2 bg-gray-300 text-black rounded-lg hover:bg-gray-400">
                                        Reset
                                    </a>
                                @if (auth()->user()->is_leader)
                                <a href="{{ URL('hamuktiskform') }}" class=" bg-green-500 text-white rounded-md px-4 py-2">{{__('Tambah')}}</a>
                                @else
                                @endif
                                </div>
                                        ...
                                        <tbody class="bg-white">
                                            ...
                                            <tr id="{{ $sk['id'] }}">
                                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                    <div class="text-sm font-medium leading-5 text-gray-900 whitespace-nowrap">{{ $sk['nomorfull'] }}
                                                    </div>
                                                ...
                                                <td class="relative mx-auto max-w-6xl [--arrow-size:5px] [--tooltip-color:white] border-b border-gray-200">
                                                    <div id="grid" class=" gap-2  p-4 ">
                                                        <div id="gridItem" class="relative cursor-pointer min-w-40 line-clamp-2 text-xs hover:z-50" data-tooltip="LOOK HERE">{{ $sk['perihal'] }}</div>
                                                    </div>
                                                ...
                                                <td class="relative mx-auto max-w-6xl [--arrow-size:5px] [--tooltip-color:white] border-b border-gray-200">
                                                    <div id="grid" class=" gap-2  p-4 ">
                                                        <div id="gridItem" class="relative cursor-pointer min-w-40 line-clamp-2 text-xs hover:z-50" data-tooltip="LOOK HERE">{{ $sk['tanggal'] }}</div>
                                                    </div>
    
                                                </td>
                                                <td class="relative mx-auto max-w-6xl [--arrow-size:5px] [--tooltip-color:white] border-b border-gray-200">
                                                    <div id="grid" class=" gap-2  p-4 ">
                                                        <div id="gridItem" class="relative cursor-pointer min-w-40 line-clamp-2 text-xs hover:z-50" data-tooltip="LOOK HERE">{{ $sk['fungsi'] }}</div>
                                                    </div>
                                                ...
                                                <td
                                                    ...
                                                    @if (auth()->user()->is_leader)
                                                    <form action="{{ route('hamuktisk.delete', $sk['id']) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus ini?');" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
...
<script type="text/javascript">
    ...
    detailButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            fetch(`/hamuktisk/${rowId}`) // Ganti URL sesuai dengan endpoint Anda
                .then(response => {
                    if (!response.ok) {
