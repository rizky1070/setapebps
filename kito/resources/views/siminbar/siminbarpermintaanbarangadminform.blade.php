...
<body>
    ...
    <a href="/siminbarpermintaanbarangadmin"
    ...
    <div class=" bg-gray-100 min-h-screen flex items-center justify-center ">
        <div class="m-8 bg-white p-8 rounded shadow-md max-w-md w-full mx-auto">
            ...
            <form
                action="{{ route('siminbarpermintaanbarangadminform.storeAdmin', ['id' => $permintaanbarang['id']]) }}"
                ...
                <!-- barang id -->
                <input id="barang_id" name="barang_id" class="hidden" value="{{ $permintaanbarang['barang_id'] }}">
                ...
                <div class="mt-4">
                    ...
                    @if (!empty($permintaanbarang['buktifoto']))
                        <img id="bukti"
                            src="{{ asset('storage/uploads/images/' . $permintaanbarang['buktifoto']) }}"
                            alt="Image" width="200">
                    @else
                        <p>-</p>
                    @endif
                ...
                <div id="signatureSection" class="mt-4 hidden">
                    <label for="signature-pad" class="block text-sm font-medium text-gray-700">Tanda Tangan
                        Admin</label>
                    ...
                    <input type="hidden" id="ttdadmin" name="ttdadmin" required>
                </div>
                <p id="notification" class="text-red-500 text-sm mt-2 hidden">Anda tidak bisa menyetujui karena stok
                    kosong.</p>
                <!-- Submit button -->
                <div class="mt-6">
...
<script>
    ...
    document.getElementById('clear').addEventListener('click', () => {
        ...
        document.getElementById('ttdadmin').value = ''; // Reset nilai input
    });
    ...
    document.getElementById('save').addEventListener('click', () => {
        if (signaturePad.isEmpty()) {
            ...
            document.getElementById('ttdadmin').value = dataUrl; // Simpan data URL ke input tersembunyi
    ...
    // Misalkan Anda mendapatkan stoktersedia dari server
    const stoktersedia = parseInt(document.getElementById('stoktersedia').value,
    10); // Ganti ini dengan nilai yang sesuai
    const signatureSection = document.getElementById('signatureSection');
    const notification = document.getElementById('notification');
    // Cek stoktersedia
    if (stoktersedia == 0) {
        // Sembunyikan signature pad
        notification.classList.remove('hidden');
        // Nonaktifkan tombol submit
        submitButton.disabled = true;
    } else {
        // Tampilkan signature pad
